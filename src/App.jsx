...
function PedidosDelDia({ isMobile }) {
  ...
  const lista = pedidos.filter((p) => parseDet(p.detalle).tipo !== "visita");
  ...
  return (
    <div style={{ padding: isMobile ? "14px 12px" : "20px 24px", background: L.bg, minHeight: "100%" }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <div style={{ fontSize: 13, color: L.muted, textTransform: "capitalize" }}>{hoyTxt} · {lista.length} {lista.length === 1 ? "pedido" : "pedidos"}</div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", color: L.light, fontSize: 14, padding: 40, background: L.white, borderRadius: 12, border: `1px solid ${L.border}` }}>
          Todavía no hay pedidos cargados hoy.
        </div>
      ) : (
...
export default function App() {
  ...
  // de dónde volver.
  const vistaConLista = vista === "chat" || vista === "contactos" || (vista === "pedidos" && rol !== "administracion");
